...
async function run() {
  try {
    ...
    if (!existingTag) {
      ...
      const newTagName = getTagName(pkg.version);
    ...
    let newReference;
    const tagName = existingTag ? existingTag.name : newTag.data.tag
    try {
      if (existingTag) {
        core.warning(
          `Updating old reference to ${tagName} SHA ${newSha}` +
            os.EOL
        );
        try {
          ...
          core.warning(`Updated tags/${tagName}` + os.EOL);
        } catch (error) {
          core.warning(
            `Unable to update old reference to tags/${tagName} SHA ${newSha} ERROR:${error.message}` +
              os.EOL
          );
        }
      }
      ...
      core.warning(
        `Unable to create new reference refs/tags/${tagName} sha ${newSha} repo ${repo}` +
          os.EOL
      );
    ...
    if (typeof newReference === "object") {
      core.setOutput("tagname", tagName);
      core.setOutput("tagsha", process.env.GITHUB_SH);
